    ...
    <div className="gap-8 lg:grid lg:grid-cols-[280px_1fr]">
      <aside className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <form className="mt-6 space-y-6 text-sm text-slate-600">
          <fieldset>
            <legend className="mb-2 text-xs uppercase tracking-[0.3em] text-slate-400">
              Categories
            </legend>
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {mockProducts.map((product) => (
            <article key={product.id} className="space-y-3 rounded-2xl border border-slate-200 p-4">
              ...
              <button className="w-full rounded-xl bg-indigo-600 py-2 text-sm font-medium text-white">
                Add to Cart
              </button>
            </article>
          ))}
