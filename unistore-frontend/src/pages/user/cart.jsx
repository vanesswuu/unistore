    ...
    <div className="gap-8 lg:grid lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="space-y-4">
        ...
        <div className="space-y-4">
          {cartItems.map((item) => (
            <article key={item.id} className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:flex-row">
      ...
      <aside className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <button className="w-full rounded-xl bg-indigo-600 py-3 text-sm font-semibold text-white">Proceed to Checkout</button>
      </aside>
    </div>
  );
}
