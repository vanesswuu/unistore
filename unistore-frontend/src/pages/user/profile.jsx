    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {profileFields.map((field) => (
              <label key={field.label} className="text-sm text-slate-500">
                {field.label}
                <input defaultValue={field.value} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-2" />
              </label>
            ))}
          </div>
        ...
        <aside className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-600">
          ...
          <button className="w-full rounded-xl border border-slate-200 px-4 py-2 font-semibold text-slate-900">Logout</button>
        </aside>
      </section>
    </div>
  );
